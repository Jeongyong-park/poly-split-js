 ...
 */
import { Vector, Vectors, Polygon, Line } from "./js/main";
...
var paintEvent = function () {
  ...
  if (showHelp) {
    ...
    ctx.fillText(
      "Selected Polygon: " + selectedPolygon + " / " + polygons.length,
      10,
      (y += dy)
    );
    ctx.fillText("Selected Vertex: " + selectedPoint, 10, (y += dy));
    ctx.fillText(
      "Q/W - increase/decrease cut area square to 100",
      10,
      (y += dy)
    );
    ctx.fillText(
      "q/w - increase/decrease cut area square to 10",
      10,
      (y += dy)
    );
    ...
    ctx.fillText(
      "Left mouse click and drag'n'drop on background to move all scene",
      10,
      (y += dy)
    );
    ctx.fillText(
      "Left mouse click and drag'n'drop on vertex to move vertices",
      10,
      (y += dy)
    );
    ctx.fillText("Right mouse click to select nearest polygon", 10, (y += dy));
    ctx.fillText(
      "Middle mouse click to split edge of selected polygon",
      10,
      y + dy
    );
  }
...
var mouseMoveEvent = function (e) {
  if (mouseLeftPress && selectedPolygon != -1 && selectedPoint != -1) {
    polygons[selectedPolygon].get(selectedPoint).x =
      polygons[selectedPolygon].get(selectedPoint).x +
      (e.clientX - mouse_x) / scale;
    polygons[selectedPolygon].get(selectedPoint).y =
      polygons[selectedPolygon].get(selectedPoint).y +
      (e.clientY - mouse_y) / scale;
  ...
  mouse_y = e.clientY;
  mouse = new Vector(
    (e.clientX - offset_x) / scale,
    (e.clientY - offset_y) / scale
  );
...
var keyPressEvent = function (e) {
  ...
  if (e.keyCode == 65 /*A*/) {
    if (selectedPolygon > 0) selectedPolygon--;
    paint();
  }

  if (e.keyCode == 83 /*S*/) {
    if (selectedPolygon < Math.round(polygons.length - 1)) selectedPolygon++;
    paint();
  }
...
document.onkeyup = keyPressEvent.bind(this);
document.getElementById("cvs").onwheel = wheelEvent.bind(this);
document.getElementById("cvs").onmousemove = mouseMoveEvent.bind(this);
document.getElementById("cvs").onmouseup = mouseReleaseEvent.bind(this);
document.getElementById("cvs").onmousedown = mousePressEvent.bind(this);
document.getElementById("cvs").oncontextmenu = function () {
  return false;
};
var paint = paintEvent.bind(this);
initPolygons();
paint();
