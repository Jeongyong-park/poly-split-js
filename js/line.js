...
class Line {
  ...
  initFromVector(start, end) {
    if (!start instanceof Vector)
      throw new Error("param start was not Vector Type");
    if (!end instanceof Vector)
      throw new Error("param end was not Vector Type");
  ...
  initFromNumbers(A, B, C) {
    if (
      ...
      if (Math.abs(A) <= POLY_SPLIT_EPS && Math.abs(B) >= POLY_SPLIT_EPS) {
        ...
        Math.abs(B) <= POLY_SPLIT_EPS &&
        Math.abs(A) >= POLY_SPLIT_EPS
      ) {
        this.start.x = -(C / A);
        this.start.y = -1000;
  ...
  reverse() {
    return this.initFromVector(this.end, this.start);
  }
  ...
  getSegmentNearestPoint(point) {
    ...
    if (u < 0) return this.start;
    else if (u > 1) return this.end;
    else return this.start.addition(dir.multiplication(u));
  }

  pointSide(point) {
    var s = this.A.multiplication(point.x - this.start.x).addition(
      B.multiplication(point.x - this.start.y)
    );
  ...
  crossLineSegment(line) {
    ...
    return {
      ...
      value:
        Line.inside(
          result.x,
          Line.minimum(line.start.x, line.end.x),
          Line.maximum(line.start.x, line.end.x)
        ) &&
        Line.inside(
          result.y,
          Line.minimum(line.start.y, line.end.y),
          Line.maximum(line.start.y, line.end.y)
        ),
    };
  }
  ...
  crossSegmentSegment(line) {
    ...
    return {
      ...
      value:
        Line.inside(
          result.x,
          Line.minimum(this.start.x, this.end.x),
          Line.maximum(this.start.x, this.end.x)
        ) &&
        Line.inside(
          result.y,
          Line.minimum(this.start.y, this.end.y),
          Line.maximum(this.start.y, this.end.y)
        ) &&
        Line.inside(
          result.x,
          Line.minimum(line.start.x, line.end.x),
